...
dados_bakery <- read.transactions(con, sep=",")
...
**Resposta:** <!-- Escreva sua resposta abaixo -->
O gráfico nos permite ter uma noção mais fácil da relação quantitiva entre os itens presentes em todas as transações. Ele pode nos ajudar a configurar alguns parametros do algoritmo apriori, como o suporte e a confiança. Olhando para o histograma podemos dizer que não podemos usar um valor de suporte em torno de 10%, pois muitos itens no conjunto dos 30 mais comprados seriam ignorados pelo algoritmo. Como temos itens com grande frequência relativa, existe uma chance forte de existirem grandes relações entre os itens, por isso provavelmente não vamos precisar utilizar valores pequeno de confiança. 
(não sei se temos que falar sobre min e max lenght, mas se acharmos uma boa já tá aí>)
Sobre o tamanho mínimo e máximo das relações, como a maior parte das transações possuem entre 2 a 4 itens, não precisamos nos ater a intervalor mais amplos que esse.
<!-- Fim da resposta -->
