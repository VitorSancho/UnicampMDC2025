...
```{r atv2-code}
# Conjunto 1: suporte = 0.02 e confiança = 0.4
...
# Conjunto 2: suporte = 0.015 e confiança = 0.25
conjunto2 <- apriori(dados_bakery,
                     parameter = list(support = 0.015, confidence = 0.25))
...
Para o primeiro conjunto não foi retonada nenhuma regra, o que indica que foi muito restritiva dado que o conjunto de dados é dominado por itens com grande distorção na frequencia dos itens comprados.
Para o segundo conjunto tivemos como resultado regras que estão muito associadas com a compra do item mais frequente nas transações, o café. Além disso, maior parte das top 5 regras, tanto do ponto de vista de suporte quanto de confiança, possuem somente dois itens. O que indica que os valores de confiança de suporte utilizados ainda foram muito restritivos para essa base dados, dominada por um desbalanceamente em termos de frequencia relativa de itens. Para complementar a resposta, já fizemos uma analise inicial do lift nessa questão também. Observando as top 5 de maior lift podemos notar que quando clientes compras duas bebidas, como café e chá, ou café e chocolate quente, existe uma chance grande de comprar um bolo, o que pode indicar que duas pessoas são capazes de dividir esse bolo para comerem juntas. Nota-se também uma forte relação com a compra de torradas com café.
Já para o terceiro conjunto tivemos resultados mais genéricos e amplos, dados os limites menos restritivos aplicados pelos limites de suporte e confiança. Nessas condições, nos top 5 com maior confiança já podemos observar a presença de transações com 3 itens. Porém nos top 5 com maior suporte tivemos como resultado transações com apenas 1 item, o que não nos traz informações sobre dependência entre itens, indicando que café, pão e chá são, com grande frequencia, comprados de forma isolada, sendo itens únicos de uma compra. 
Com a ordenação por lift é possível notar relações interessante. Existe uma grande chance de que um cliente compre sanduíche para acompanhar uma coca, o que pode indicar um hábito para horários de lanche ou até almoços e jantas. O que é curioso é a relação entre a compra de sanduíchea e sopa, que aparece em 3 das 5 top transções com maior lift.
**Resposta:** <!-- Escreva sua resposta abaixo -->
...
quality(conjunto3) <- cbind(quality(conjunto3), interestMeasure(conjunto3, measure=c("conviction", "oddsRatio"), 
                                          transactions = transacoes))
