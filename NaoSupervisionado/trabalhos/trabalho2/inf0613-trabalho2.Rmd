...
wss <- sapply(1:30, function(k){
  kmeans(dados_reduzidos, centers=k)$tot.withinss
})
...
sil <- sapply(2:30, function(k){
  km <- kmeans(dados_reduzidos, centers=k)
...
k_final <- 17
set.seed(123)
km_final <- kmeans(dados_reduzidos, centers=k_final, nstart=25)
...
ggplot(dados_plot, aes(x=PC1, y=PC2, color=cluster, label=speech$LETRA)) +
  geom_text(size=3) +
  labs(title=paste("K-means com K =", k_final),
       x="PC1", y="PC2") +
  theme_minimal()
...
Como o algoritmo PCA não conseguiu prover uma simplificação dos dados com forte separação, ao aplicarmos o kmeans e trazer de volta para 2 dimensões, o resultado do agrupamento não ficou simples de interpretar. Escolhemos K = 17 pois apresenta um valor mais alto de silhueta, com a distância intra-cluster um pouco menor. A escolha de valor para K não foi algo muito claro, pois o decréscimo de distancias na curva WSS x K se comportou de forma relativamente constante. Isso indica que não tivemos um valor de K com muito potencial para agrupamento. Pelo gráfico, conseguimos observar que os clusters capturaram alguma estrutura em alta dimensão, pois algumas letras estão agrupadas de forma coerente, como o 'R' e o 'O', mas no geral a visualização dos dados ainda está bem prejudicada.
...
for (eps in eps_values) {
  for (minPts in minPts_values) {
    model <- dbscan(dados_reduzidos, eps = eps, minPts = minPts)
    clusters <- model$cluster

    if (length(unique(clusters[clusters != 0])) >= 2) {
      sil <- silhouette(clusters, dist(dados_reduzidos))
      silhouette_avg <- mean(sil[, 3], na.rm = TRUE)
    } else {
      silhouette_avg <- NA
    }
    ...
    results <- rbind(results, data.frame(
      eps = eps,
      minPts = minPts,
      clusters_found = length(unique(clusters[clusters != 0])),
...
results_sorted <- results[order(-results$silhouette), ]
...
best_minpts <- 45
kNNdistplot(dados_reduzidos, k = best_minpts)
...
best_minpts <- 45
best_eps <- 6
dbscan_final <- dbscan(dados_reduzidos, eps = best_eps, minPts = best_minpts)

# Prepara dados para plotar (apenas 2 PCs para visualização)
dados_plot <- data.frame(PC1 = dados_reduzidos[,1],
                         PC2 = dados_reduzidos[,2],
                         cluster = factor(dbscan_final$cluster))

# Construindo um gráfico de dispersão
ggplot(dados_plot, aes(x=PC1, y=PC2, color=cluster, label=speech$LETRA)) +
  geom_text(size=3) +
  labs(title=paste("DBSCAN com eps =", best_eps, " minPts =", best_minpts),
       x="PC1", y="PC2") +
  theme_minimal()
...
Pelo grid search feito no item b, identificamos minPts = 45 como o parâmetro que gera o melhor valor de silhueta. Ao analisar o knnDistPlot, escolhemos o valor de eps = 6 e validamos com o plot 2d dos clusters. Conseguimos 5 clusters e 2032 pontos identificados como ruído. Quanto à qualidade do agrupamento, ao incluir a informação dos labels de cada ponto, vemos que o agrupamento não foi bom, pois juntou letras que não fazem sentido juntas, como 'H', 'Z' e  'K' (cluster 1). Neste sentido, parece que o algoritmo K-means conseguiu uma performance um pouco melhor, apesar de ambos terem dificuldades de encontrar um bom agrupamento no espaço dimensional encontrado pelo PCA.
<!-- Fim da resposta -->
