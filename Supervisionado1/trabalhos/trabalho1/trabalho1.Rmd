...
train_pred <- predict(baseline, treino_normalizado)
valid_pred <- predict(baseline, validacao_normalizado)
```
...
mse_train <- MSE(train_pred, treino_normalizado$Calorias)
r2_train  <- R2(train_pred, treino_normalizado$Calorias)

# Calculando as métricas para o conjunto de validação
mae_valid <- MAE(valid_pred, validacao_normalizado$Calorias)
mse_valid <- MSE(valid_pred, validacao_normalizado$Calorias)
r2_valid  <- R2(valid_pred, validacao_normalizado$Calorias)
...
Abaixo é possível observar a matriz de correlação entre as features do data set.

```{r}
# Calculando a matriz de correlação
correlation <- cor(treino_normalizado[, 1:8])
print(correlation)
```
Ao observar os dados é possível notar que existe uma correçãoa forte entre as features Altura e Peso, tornando-as candidatas para uma combinação.
Porém é importante também realizar uma analise visual da correlação, pois ela permite observar mais features que podem ser combinadas:
...
```
No plot acima é possível notar que mais fatures podem ser combinadas, pois possuem um relativa correlação. As features Duração e Temperatura.Corporal tem uma grau de correção de 0.90. O valor de correlação de 0.85 é observado entre Duração e Batimento.Cardiado, e 0.77 entre Batimento.Cardiaco e Temperatura.Corporal. Essas três são grande candidatas a serem combinadas numa só, além de ser possível combinar um par delas em somente uma.
