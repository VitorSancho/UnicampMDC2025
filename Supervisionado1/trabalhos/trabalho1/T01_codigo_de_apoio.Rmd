...
paste("O dataframe de TREINO possui",nrow(dados_treino_sem_NAs),
      "registros após a limpeza",sep=" ")

paste("summary TREINO:")
summary(dados_treino_sem_NAs)

paste("O dataframe de VALIDAÇÃO possui",nrow(dados_validacao_sem_NAs),
      "registros após a limpeza",sep=" ")
...
ggplot(dados_treino_sem_NAs, aes(x = Duracao)) +
  geom_histogram(fill = "orange", color = "black") +
  theme_minimal() +
  labs(title = "Distribuição do duração do exercício",
       x = "Duração", y = "Densidade")

# Histograma do batimetno cardiaco
ggplot(dados_treino_sem_NAs, aes(x = Batimento.Cardiaco)) +
  geom_histogram(fill = "orange", color = "black") +
  theme_minimal() +
  labs(title = "Distribuição do batimento cardiaco",
       x = "Duração", y = "Densidade")
...
ggplot(dados_treino_sem_NAs, aes(x = Temperatura.Corporal)) +
  geom_histogram(fill = "orange", color = "black") +
  theme_minimal() +
  labs(title = "Distribuição da temperatura corporal", 
       x = "Duração", y = "Densidade")
...
correlation <- cor(treino_normalizado)


corrplot(correlation, method = "color", type = "upper",
         addCoef.col = "grey50", tl.col = "black", tl.srt = 45, number.cex = 0.6, tl.cex = 0.6)


# Análise de Feature x Variável, para observar o comportamento da influência de 
# cada feature na variável alvo.
ggplot(dados_treino_sem_NAs, aes(x = Peso, y = Calorias)) + 
  geom_point() + geom_smooth()
ggplot(dados_treino_sem_NAs, aes(x = Altura, y = Calorias)) + 
  geom_point() + geom_smooth()
ggplot(dados_treino_sem_NAs, aes(x = Duracao, y = Calorias)) + 
  geom_point() + geom_smooth()
ggplot(dados_treino_sem_NAs, aes(x = Batimento.Cardiaco, y = Calorias)) + 
  geom_point() + geom_smooth()
ggplot(dados_treino_sem_NAs, aes(x = Temperatura.Corporal, y = Calorias)) + 
  geom_point() + geom_smooth()
ggplot(dados_treino_sem_NAs, aes(x = Genero, y = Calorias)) + 
  geom_point()
ggplot(dados_treino_sem_NAs, aes(x = Idade, y = Calorias)) + 
  geom_point()
```
...
Métricas do modelo 8 (melhor dentre os 17 avaliados):

```{r}
model_metrics(treino_normalizado, validacao_normalizado, models_combination[[8]])
```
...
# Lista completa de fórmulas
formulas <- c(f_pol01, f_pol02, f_pol03, f_pol04, f_pol05, 
              f_pol06, f_pol07, f_pol08, f_pol09, f_pol20, f_pol50)
...
p <- ggplot(data=MaePerDegreeMelt, aes(x=Formula, y=value, colour=variable)) + 
  geom_line() + geom_point()

p <- p + ggtitle("Curva vies/variancia") + ylab("MAE") + 
  scale_x_discrete(name ="Ordem Polinomial (10 = grau 20 e 11 = grau 50)", 
...
Com base no MAE, o melhor modelo encontrado até o momento foi o f08, gerado a partir das combinações de features simples. Regressão polinomial para o modelo com as features combinadas, usando como referencia a feature combinada f03:
...
Analisando o conjunto de teste, vemos que ele tem um total de 2251 registros e não contém registros inválidos, ou seja, com valores NA.

Em seguida, fazemos a normalização e one-hot encoding de dados categóricos no conjunto de teste.
...
test_normalized[, 1:6] <- as.data.frame(lapply(test_normalized[, 1:8], 
                                               function(x) pmax(0, pmin(1, x))))
summary(test_normalized)
```

Métricas do melhor modelo (f08) avaliado no **conjunto de teste**:

```{r atv5-code}

best_model <- lm(formula = f08, data=treino_normalizado)

model_metrics(treino_normalizado, test_normalized, best_model)
```
...
**Resposta:** <!-- Escreva sua resposta abaixo -->
Avaliamos, dentre todos os modelos desenvolvidos no trabalho, o que apresentou menor MAE de validação. O modelo f08 ganhou neste comparativo, com MAE_validacao = 0.01977. Também chamou a atenção, a simplicidade do modelo frente a sua performance, dado que testamos modelos muito mais complexos matematicamente e, ainda assim, este foi o que apresentou a melhor performance.
<!-- Fim da resposta -->
...
**Resposta:** <!-- Escreva sua resposta abaixo -->
O valor de MAE obtido nos conjuntos de validação e de teste foram de 0.01977 e 0.02005, respectivamente. Os valores são numericamente muito
próximos, mas o MAE no conjunto de teste indica um maior erro para esse conjunto de dados. É necessária uma análise mais profunda, mas
seria uma boa abordagem avaliar esse comportamente para um conjunto maior de dados de testes, a fim de determinar
a capacidade de generalização do modelo. Se o erro no conjunto de teste aumentar para uma nova batelada de dados podemos inferir que o
modelo possui overfitting.
<!-- Fim da resposta -->

5.3 -- Expliquem a diferença entre os modelos e o porquê que estas diferenças levaram a resultados piores ou melhores.

**Resposta:** <!-- Escreva sua resposta abaixo -->
Ao longo dos testes, buscamos variar o uso das diferentes features, não só em termos de graus polinomiais, mas também em termos da presença ou não das features no modelo. No caso do modelo f08, ele se destaca pela simplicidade frente a todos os outros modelos polinomiais que testamos. Seus parâmetros dependem apenas da observação categórica da idade e de uma combinação de terceira ordem entre os parâmetros Duracao, Batimentos Cardiacos e Temperatura Corporal. Este comportamento corrobora com a análise preliminar de correlação que fizemos, que sugere que estas três features estão altamente correlacionadas entre si e têm alta correlação com a variável alvo.
<!-- Fim da resposta -->
...
**Resposta:** <!-- Escreva sua resposta abaixo -->
Para melhorar ainda mais o modelo pode-se testar como ele se comportaria ao aplicar a hipótese de ajuste polinomial dos dados,
observando como o MAE se alteraria conforme o aumente de grau do polinômio. Outra abordagem seria testar a normalização dos dados
utilizando outra metodologia, como o z-score, por exemplo.
<!-- Fim da resposta -->
