...
```{r}
# One hot encoding para conjunto de TREINO
...
# One hot encoding para conjunto de VALIDAÇÃO
...
PrecisionPorGraus <- data.frame(graus=numeric(6), TrainPrecision=numeric(6), ValidPrecision=numeric(6))
RecallPorGraus <- data.frame(graus=numeric(6), TrainRecall=numeric(6), ValidRecall=numeric(6))
...
getModelMetrics <- function(model, training_set, validation_set){
  ...
  model_valid_pred <- predict(model, validation_set, type="class")
...
Como o modelo ótimo é baseado em árvore de decisão, não precisaremos fazer nenhum tipo de encoding nas features categóricas do conjunto de teste, sendo necessário apenas aplicar a normalização que aplicamos nas etapas de treino e validação.
...
dados_teste_normalizado$Risco.Hipertensao <- as.factor(dados_teste_normalizado$Risco.Hipertensao)

# Predição no conjunto de teste

melhor_modelo <- optimal_rf_model
resultado_final <- getModelMetrics(melhor_modelo, valid_normalizado_rf, dados_teste_normalizado)
resultado_final$train_cm
resultado_final$valid_cm
resultado_final$metrics
```
...
Avaliando uma combinação das 4 métricas de classificação geradas ao longo do trabalho, o modelo completo com profundidade 5 e o modelo de random forest com 10 árvores tem desempenhos muito próximos, com o random forest sendo ligeiramente superior. Todas as métricas são superiores, exceto o Recall, que é menos de 1% inferior no Random Forest, comparado à arvore simples com profundidade 5. Por este motivo, escolhemos o modelo Random Forest para utilizar no teste final.
...
O resultado no conjunto de teste é consideravelmente pior do que no conjunto de treinamento, o que sugere algum grau de overfitting. No entanto, curiosamente, o recall foi uma métrica que melhorou em comparação com a validação do modelo. No geral, observamos uma redução de mais de 10%, em termos absolutos, entre as métricas na validação e no teste. Neste cenário, podemos afirmar que houve uma forte degradação do modelo ao ir para "produção".
...
Ambos modelos são similares, com a diferença da estratégia de random forest. Inclusive, ambos têm restrições similares em relação à profundidade máxima das árvores, mas o algoritmo de random forest consegue capturar mais nuances do modelo, por treinar diversas árvores (10, no nosso caso) sobre subconjuntos dos dados.
...
Poderíamos tentar calibrar melhor a quantidade de features usadas no Random Forest e, com isso, alterar a quantidade de árvores, com a hipótese de que uma quantidade maior árvores mais simples pode ser melhor que uma menor de árvores complexas. Uma outra alternativa mais óbvia, porém não necessariamente simples, seria coletar mais dados de treinamento, já que um conjunto de cerca de 1300 exemplos pode não ser suficiente para que o modelo aprenda a generalizar tão bem.
