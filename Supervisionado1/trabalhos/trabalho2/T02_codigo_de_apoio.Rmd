...
Levando isso em consideração, no conjunto de treinamento tivemos cerca de 77% de recall. O percentual caiu para 78% na validação, o que nos permite inferir que não ocorreu overfitting  no modelo. A precisão ficou em torno de 55%, superior ao recall, mas não vamos nos precupar com essa métrica, pois ela penaliza a presença de falsos positivos - e como dito anteriormente, não temos que temer dizer que uma pessoa tem risco quando na verdade não tem.
Com esses desempenho o modelo se mostra capaz de prever bem os casos possível de hipertensão, mas ainda podemos melhorar o modelo para que o recall melhore e passemos a classificar corretamente os indivíduos com maior risco.
<!-- Fim da resposta -->
...
matriz_correlacao <- cor(train_data[,1:4][ , 1:(ncol(train_data[,1:4]) - 1)])
...
PrecisionPerCombination <- data.frame(combination=numeric(6), TrainLoss=numeric(6), ValidLoss=numeric(6))
RecallPerCombination <- data.frame(combination=numeric(6), TrainLoss=numeric(6), ValidLoss=numeric(6))
LossPerCombination <- data.frame(combination=numeric(6), TrainLoss=numeric(6), ValidLoss=numeric(6))
F1PerCombination  <- data.frame(combination=numeric(6), TrainF1=numeric(6), ValidF1=numeric(6))
...
for(hypothesis in formulas){  
  ...
  PrecisionPerCombination[i,]  <- c(i, train_metrics["Precision"], valid_metrics["Precision"]) 
  RecallPerCombination[i,]  <- c(i, train_metrics["Recall"], valid_metrics["Recall"]) 
...
# Gráfico da Precisão
PrecisionPerCombinationMelt <- melt(PrecisionPerCombination, id="combination") 
p <- ggplot(data=PrecisionPerCombinationMelt, aes(x=combination, y=value, colour=variable)) + geom_line() + geom_point()

p <- p + ggtitle("Curva vies/variancia") + ylab("Precision") + scale_x_discrete(name ="Combination", limits=as.character(1:length(formulas)))

p + theme(legend.position = c(0.7, 0.50), legend.title = element_blank())

# Gráfico da Recall
RecallPerCombinationMelt <- melt(RecallPerCombination, id="combination") 
p <- ggplot(data=RecallPerCombinationMelt, aes(x=combination, y=value, colour=variable)) + geom_line() + geom_point()

p <- p + ggtitle("Curva vies/variancia") + ylab("Recall") + scale_x_discrete(name ="Combination", limits=as.character(1:length(formulas)))
...
Fizemos a análise de correlação entre as features contínuas e as discretas. Nas duas não foi possível observar uma forte correlação entre 2 ou mais features. Por isso não pudemos agrupar e assim reduzir o total de features do modelo.
...
```{r}
# Predicao do modelo
baseline2_train_pred <- predict(baseline2, treino_normalizado, type="class")
baseline2_valid_pred <- predict(baseline2, validacao_normalizado, type="class")

# # gera o Loss para as bases de treino e validação
# baseline2_train_losses <- getLoss(treino_normalizado$Risco.Hipertensao, baseline2_train_pred)
# baseline2_valid_losses <- getLoss(validacao_normalizado$Risco.Hipertensao, baseline2_valid_pred)

# Calcula a matriz de confusão
baseline2_train_cm <- confusionMatrix(data=as.factor(baseline2_train_pred), 
                      reference=as.factor(treino_normalizado$Risco.Hipertensao), 
                      positive='1')

baseline2_valid_cm <- confusionMatrix(data=as.factor(baseline2_valid_pred), 
                      reference=as.factor(validacao_normalizado$Risco.Hipertensao), 
                      positive='1')


# Calculando as métricas para o conjunto de treino
baseline2_train_metrics = calculateMetrics(baseline2_train_cm$table)

# Calculando as métricas para o conjunto de validação
baseline2_valid_metrics = calculateMetrics(baseline2_valid_cm$table)

# Organizando os resultados em um dataframe
results_baseline2 <- data.frame(
  Train  = baseline2_train_metrics,
  Valid  = baseline2_valid_metrics
)

# Formatando os resultados para melhor leitura
results_baseline2$Train <- as.numeric(results_baseline2$Train)
results_baseline2$Valid <- as.numeric(results_baseline2$Valid)

# Evitando notação científica na exibição dos resultados
results_baseline2$Train <- format(results_baseline2$Train, scientific = FALSE)
results_baseline2$Valid <- format(results_baseline2$Valid, scientific = FALSE)
...
AccPerDepth  <- data.frame(depth=length(depths), TrainAcc=length(depths), ValidAcc=length(depths))
F1PerDepth  <- data.frame(depth=length(depths), TrainF1=length(depths), ValidF1=length(depths))

for(i in depths){
  depth_tree_model <- rpart(Risco.Hipertensao ~ Consumo.Sal + Nivel.de.Estresse + 
                      Historico.Pressao.Arterial + Duracao.Sono + IMC +
                      Historico.Familiar + Nivel.Exercicio + Fumante, 
                    data=treino_normalizado,
                    method="class",
  ...
  depth_train_pred <- predict(depth_tree_model, treino_normalizado, type="class")
  depth_valid_pred <- predict(depth_tree_model, validacao_normalizado, type="class")
...
F1PerDepthMelt <- melt(F1PerDepth, id="depth")
